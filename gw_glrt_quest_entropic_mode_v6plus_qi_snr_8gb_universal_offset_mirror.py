...
- Podepsaná koherence H1×L1: coh_rho (−1..+1) po stejném okně (citlivé na mirror test)
...
# --- signed coherent overlap H1×L1 (po BP/whiten a po stejném výřezu) ---
def compute_coh_rho(h: np.ndarray, l: np.ndarray) -> float:
    """Vrátí podepsanou koherenční korelaci (−1..+1) mezi H1 a L1.
       Očekává stejné délky a už zpracované (BP/whiten + crop)."""
    h = np.asarray(h).ravel()
    l = np.asarray(l).ravel()
    if h.size != l.size or h.size == 0:
        return float('nan')
    hs = h.std(); ls = l.std()
    if hs == 0 or ls == 0:
        return float('nan')
    h = (h - h.mean()) / (hs + 1e-12)
    l = (l - l.mean()) / (ls + 1e-12)
    num = float(np.dot(h, l))
    den = float(np.sqrt(np.dot(h, h) * np.dot(l, l) + 1e-24))
    return num / den
...
def main():
    try:
        ap = argparse.ArgumentParser(description="Ω-GLRT v6+ (QI, π/3 echo, SNR, coh_rho) — univerzální načítání H1/L1[/V1]")
        # zdroje dat
        ...
        ap.add_argument("--t0_offset", type=float, default=0.0,
                help="časový posun okna v sekundách vzhledem k mergeru (pro off-source testy, např. +8 s)")
        ...
        # --- vyřízni okno s volitelným posunem t0_offset (pro off-source testy) ---
        def take_segment(x: np.ndarray, fs: float, dur: float, t0_offset: float = 0.0) -> np.ndarray:
            need = int(round(dur * fs))
            start = len(x)//2 - need//2 + int(round(t0_offset * fs))
            start = max(0, min(start, len(x) - need))
            return x[start:start + need]

        h = take_segment(h, fs, max(args.duration, 0.1), getattr(args, "t0_offset", 0.0))
        l = take_segment(l, fs, max(args.duration, 0.1), getattr(args, "t0_offset", 0.0))
        if v is not None:
            v = take_segment(v, fs, max(args.duration, 0.1), getattr(args, "t0_offset", 0.0))
        ...
        # ---- Koherentní korelace H1×L1 (po stejném okně a zpracování) ----
        coh_rho = None
        if len(dets) >= 2:
            try:
                h_np = dets[0].y.detach().cpu().numpy()
                l_np = dets[1].y.detach().cpu().numpy()
                coh_rho = compute_coh_rho(h_np, l_np)
                print(f"[COH] rho(H1×L1) = {coh_rho:+.3f}")
            except Exception as e:
                print(f"[COH] rho(H1×L1) = NaN ({e})")
                coh_rho = None
        ...
        # meta info (off-source, negace L1)
        meta = {"t0_offset_s": float(getattr(args, "t0_offset", 0.0))}
        if args.l1 and args.l1.upper().endswith("_NEG.TXT"):
            meta["l1_negated"] = True
        ...
        summary = {
            ...
            "coh_rho": (None if coh_rho is None else float(coh_rho)),
            "meta": meta
        }
