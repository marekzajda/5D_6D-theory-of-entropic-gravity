...
def _resample_to_fs(arr: np.ndarray, fs_src: float, fs_dst: float) -> np.ndarray:
    if abs(fs_src - fs_dst) < 1e-6:
        return arr.astype(np.float32, copy=False)
    try:
        from scipy.signal import resample_poly
        # rozumný poměr (polyphase)
        up = int(round(fs_dst)); dn = int(round(fs_src))
        return resample_poly(arr, up, dn).astype(np.float32, copy=False)
    except Exception:
        # nouzově lineární
        t_src = np.arange(len(arr))/fs_src
        n_dst = int(round(len(arr)*fs_dst/fs_src))
        t_dst = np.arange(n_dst)/fs_dst
        return np.interp(t_dst, t_src, arr).astype(np.float32, copy=False)

def load_timeseries_hdf5(path: str, fs_hint: float) -> np.ndarray:
    if not HAVE_H5PY:
        raise RuntimeError("Chybí h5py: nainstaluj `pip install h5py`.")

    with h5py.File(path, "r") as f:
        # GWOSC standard: strain/Strain (dataset), attrs["Xspacing"] = 1/fs
        if "strain" in f and "Strain" in f["strain"]:
            dset = f["strain"]["Strain"]
        elif "strain/Strain" in f:
            dset = f["strain/Strain"]
        else:
            # DFS: první dataset obsahující 'Strain' nebo 'strain'
            found = []
            def dfs(g):
                for k, v in g.items():
                    if isinstance(v, h5py.Dataset) and ("Strain" in k or "strain" in k):
                        found.append(v)
                    elif isinstance(v, h5py.Group):
                        dfs(v)
            dfs(f)
            if not found:
                raise RuntimeError("V HDF5 nenalezen dataset 'strain/Strain'.")
            dset = found[0]
        ...
        if "Xspacing" in dset.attrs:
            dx = float(dset.attrs["Xspacing"])
            if dx > 0:
                fs_src = 1.0 / dx
        if fs_src is None:
            try:
                fs_src = float(f["meta"]["SampleRate"][()])
            except Exception:
                fs_src = float(fs_hint)

    # stejné chování jako TXT loader: jen bezpečné čísla; normalizace je až později
    data = np.nan_to_num(data, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32, copy=False)
    # pokud by hdf5 nebylo na --fs, přeresamplujeme na fs_hint
    data = _resample_to_fs(data, fs_src, float(fs_hint))
    return np.ascontiguousarray(data, dtype=np.float32)

def load_timeseries_auto(path: str, fs_hint: float) -> np.ndarray:
    if path is None:
        return None
    pl = path.lower()
    if pl.endswith(".hdf5") or pl.endswith(".h5"):
        return load_timeseries_hdf5(path, fs_hint=fs_hint)
    # default: původní TXT loader
    return load_timeseries_txt(path)
...
def main():
    try:
        ...
        h = load_timeseries_auto(H_path, fs_hint=fs)
        l = load_timeseries_auto(L_path, fs_hint=fs)
        v = load_timeseries_auto(V_path, fs_hint=fs) if V_path else None
